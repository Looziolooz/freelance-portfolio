...
export const MacbookScroll = ({
  ...
  const scaleX = useTransform(scrollYProgress, [0, 0.3], [1.2, isMobile ? 1 : 1.4]);
  const scaleY = useTransform(scrollYProgress, [0, 0.3], [0.6, isMobile ? 1 : 1.4]);
  // The screen detaches and slides down to become a readable slide. Capped at
  // ~760px so the enlarged card stays fully in frame (its bottom no longer gets
  // clipped by the viewport) through the middle of the scroll.
  const translate = useTransform(scrollYProgress, [0, 1], [0, 760]);
  ...
  return (
    ...
    >
      <motion.h2
        ...
        className="mb-20 text-center text-3xl font-medium md:text-5xl"
      >
        {title || (
          <span>
            Questo l&apos;ho costruito io. Davvero.
          </span>
        )}
      </motion.h2>
      {/* Lid */}
      <Lid src={src} screen={screen} scaleX={scaleX} scaleY={scaleY} rotate={rotate} translate={translate} />
      {/* Base area */}
  ...
  return (
    <div className="relative [perspective:800px]">
      ...
      <motion.div
        ...
        className={`absolute inset-0 h-96 w-[32rem] rounded-2xl p-2 ${screen ? "" : "bg-[#010101]"}`}
      >
        {/* Custom screen fills the lid edge-to-edge (no dark bezel) so the frame
            doesn't peek as faint lines above/below. The image variant keeps the
            classic dark MacBook bezel. */}
        {screen ? (
          <div className="absolute inset-0 overflow-hidden rounded-2xl">{screen}</div>
        ) : (
          <>
...
// Brand mark on the closed lid (replaces the Aceternity logo).
const LidMark = () => {
  return (
    <span
      style={{
    ...
    >
      LO<span style={{ color: "var(--accent-peach)" }}>.</span>oz
    </span>
  );
};
