// Vedi l'intestazione dentro la stringa per cosa c'e' e cosa manca, e perche'.
export const SC_ENGINE_CSS = `
/* Layer DEVICES di scrollcraft.css, preso testuale dall'engine della skill
   scroll-craft (nateherkai, MIT). Qui manca DI PROPOSITO tutto il resto del
   file: i token, il reset, html/body e la scala tipografica sono il "taste
   floor" per le pagine standalone, e questo sito ha gia' il suo (DESIGN.md);
   in piu' html{scroll-behavior:smooth} litigherebbe con Lenis e body{background}
   ridipingerebbe la pergamena. Il meccanismo (stage, pin, cue, split, reduce,
   mobile) e' intatto: e' la parte che il file stesso dice di non ritoccare.
   I token --sc-* che il meccanismo legge sono rimappati sotto sul brand. */
:root {
  /* Meccanismo (valori ORIGINALI dell'engine: easing, durate, z, misure). */
  ...
  /* Brand (i sei ruoli colore che il file dice di sostituire). */
  --sc-canvas:   #1B1813;
  --sc-ink:      #F4F1EA;
  --sc-ink-soft: #A89E8B;
  --sc-accent:   var(--accent-green, #E8A12C);
