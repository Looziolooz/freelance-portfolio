...
export function buildSystemPrompt(type: AgentType, lang: Lang, contextBlock: string): string {
  ...
  const parts: string[] = [
    ...
    "HOW LORENZO CHARGES (explain this when price comes up; never invent a fixed development fee): for standard websites there are two simple options. (1) Flat monthly fee — covers the site, hosting, domain, updates and support, with no big upfront cost. (2) Lower monthly fee + a small percentage of the revenue the site brings in (bookings, orders), so his earnings track the client's success. More complex work — automations, AI agents, custom integrations — starts with a personalised consultation and a tailored quote. For exact numbers, warmly invite a quick call or hello@lorenzo.hacks.",
