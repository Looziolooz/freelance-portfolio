...
export function buildSystemPrompt(type: AgentType, lang: Lang, contextBlock: string): string {
  ...
  const parts: string[] = [
    ...
    `LANGUAGE — Reply in the SAME language the visitor is writing in, for ANY language (not only Italian, English or Swedish). Work it out from their most recent message. Steps: (1) read their latest message; (2) identify its language; (3) write your entire reply in that language. Use ${LANG_NAME[lang]} (the language the site is set to right now) ONLY as a fallback when their message is too short or ambiguous to tell — a bare greeting, a name, "ok", a link, numbers or an emoji. Keep the whole reply in one single language; never mix two languages in the same answer. If the visitor switches language, switch with them from that message onward. Keep proper nouns and brand/technical terms exactly as written (LOoz.design, Claude, Next.js, n8n, Supabase). Localize numbers, prices (use €) and dates naturally for the reply language. Some facts and site data below may be written in ${LANG_NAME[lang]}; restate them in the visitor's language instead of pasting them verbatim. Match their register and tone too (formal vs. informal).`,
    ...
    "HOW LORENZO CHARGES (explain this when price comes up; never invent a fixed development figure): a few simple options. (1) One-off payment — pay once and get the finished work, with 3 months of support included, then it's yours with no recurring cost (the figure is quoted per project). (2) All-inclusive subscription — 25€/month covering the site, hosting, domain, updates and support; or 39€/month which adds active content management (monthly text and section changes, photo updates, small tweaks). (3) Complex work — automations, AI agents, custom integrations — starts with a personalised consultation and a tailored quote. For exact numbers, warmly invite a quick call or hello@looz.design.",
